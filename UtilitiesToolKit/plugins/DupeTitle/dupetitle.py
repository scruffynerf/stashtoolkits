...
def dupefindall():
    ...
    for count, scene in enumerate(found):
       log.debug(f"processing {scene['id']} - {scene['title']}")
       checktitle(scene)
       checkstashid(scene)
       checkurl(scene)
       checkduration(scene)
...
def checktitle(scene):
    ...
    if numberofscenes > 1 and config.durationdupe:
       checkduration(scene,filter)
       return
...
def checkduration(scene, filter):
    ...
    dupeduration_id = stash.find_tag(config.dupeduration_tagname, create=True).get("id")
    neardupeduration_id = stash.find_tag(config.neardupeduration_tagname, create=True).get("id")

    filterdur = {}
    filterdur["value"] = int(scene['files'][0]['duration'])
    filterdur["modifier"] = "EQUALS"
    filter["duration"] = filterdur
    durnumberofscenes = len(stash.find_scenes(filter, filterlimits, fragment="id"))
    if durnumberofscenes > 1 and config.dupeduration_tagname not in existing_tags:
        stash.update_scenes({
                'ids': [scene['id']],
                'tag_ids': {
                  'mode': 'ADD',
        ...
        log.info(f"{scene['id']} - {scene['title']} is not unique duration, tagged as duplicate")
    if config.neardurationdupe and durnumberofscenes < 2 and config.dupeduration_tagname not in existing_tags and config.neardupeduration_tagname not in existing_tags :
      filterdur = {}
      filterdur["value"] = int(scene['files'][0]['duration']) - config.neardupeduration_diff
      filterdur["value2"] = int(scene['files'][0]['duration']) + config.neardupeduration_diff
      filterdur["modifier"] = "BETWEEN"
      filter["duration"] = filterdur
      durnumberofscenes = len(stash.find_scenes(filter, filterlimits, fragment="id"))
      if durnumberofscenes > 1:
        stash.update_scenes({
                'ids': [scene['id']],
                'tag_ids': {
                  'mode': 'ADD',
                  'ids': [neardupeduration_id]
                  }
              })
        log.info(f"{scene['id']} - {scene['title']} is not unique duration, tagged as duplicate")
    return
...
def cleanduration(scene):
    ...
    # tag ONLY if the scene is either ALSO a title OR stashid OR url OR oshash dupe 
    if config.dupeurl_tagname not in existing_tags and config.dupeoshash_tagname not in existing_tags and config.dupestashid_tagname not in existing_tags and config.dupetitle_tagname not in existing_tags:
        # we don't care about duration dupes unless one of these is true
        if config.dupeduration_tagname in existing_tags or config.neardupeduration_tagname in existing_tags:
           stash.update_scenes({
            'ids': [scene['id']],
            'tag_ids': {
                'mode': 'REMOVE',
                'ids': [dupeduration_id, neardupeduration_id]
            }
           })
           log.info(f"{scene['id']} - {scene['title']} is not dupe url/stashid/title/oshash, untagged as duplicate duration")
    return

def checkoshash(scene):
    scene = stash.find_scene(scene['id'])
    if not scene:
       log.debug("Scene issue")
       return
    existing_tags = get_names(scene.get("tags"))
    dupeoshash_id = stash.find_tag(config.dupeoshash_tagname, create=True).get("id")

    # check number of scenes with this oshash

    # add oshash filter
    filteros = {}
    filteros["value"] = scene['oshash']
    # deprecated, use files.fingerprints
    filteros["modifier"] = "EQUALS"
    filter = {}
    filter["oshash"] = filteros
    filterlimits = {}
    filterlimits["per_page"] = 5
    ...
    # if dupe tagged now and number = 1, remove
    if numberofscenes == 1 and config.dupeoshash_tagname in existing_tags:
        stash.update_scenes({
            'ids': [scene['id']],
            'tag_ids': {
                'mode': 'REMOVE',
        ...
        log.info(f"{scene['id']} - {scene['title']} is unique oshash, untagged as duplicate oshash")
        return

    # if not dupe tagged now, and number > 1, add
    if numberofscenes > 1 and config.dupeoshash_tagname not in existing_tags:
        stash.update_scenes({
            'ids': [scene['id']],
            'tag_ids': {
                'mode': 'ADD',
        ...
        log.info(f"{scene['id']} - {scene['title']} is not unique oshash, tagged as duplicate")
        return
...
def find_duplicate_title(scene):
    result = {}
    DuplicateTitleTag = stash.find_tag(config.dupetitle_tagname, create=True).get("id")
...
def main():
    ...
    if config.oshashdupe:
        results = checkoshash(scene)

    if config.durationdupe:
        results = cleanduration(scene)
